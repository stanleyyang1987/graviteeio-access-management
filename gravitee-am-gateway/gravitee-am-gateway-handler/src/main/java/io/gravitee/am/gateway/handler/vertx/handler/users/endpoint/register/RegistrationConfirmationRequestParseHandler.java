 ...
 */
public class RegistrationConfirmationRequestParseHandler implements Handler<RoutingContext> {

    private static final Logger logger = LoggerFactory.getLogger(RegistrationConfirmationRequestParseHandler.class);
    ...
    public RegistrationConfirmationRequestParseHandler(RegisterService registerService) {
    ...
    public void handle(RoutingContext context) {
        ...
        // user registration completed, continue
        if (success != null) {
            context.next();
            return;
        }
        ...
        if (token == null) {
            redirectToErrorRegisterPage(context, "token_missing");
            return;
        }

        parseToken(token, handler -> {
            if (handler.failed()) {
                redirectToErrorRegisterPage(context, "invalid_token");
                return;
            }
    ...
    private void parseToken(String token, Handler<AsyncResult<User>> handler) {
        registerService.verifyToken(token)
                .subscribe(
                        ...
                        () -> handler.handle(Future.failedFuture(new InvalidTokenException("The register token is invalid"))));
    }

    private void redirectToErrorRegisterPage(RoutingContext context, String error) {
        try {
