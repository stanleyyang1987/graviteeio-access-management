 ...
 */
@Component
public class EmailServiceImpl extends AbstractService implements EmailService, EventListener<EmailEvent, Email> {
    ...
    @Override
    public void onEvent(Event<EmailEvent, Email> event) {
        if (enabled) {
            send(event.content());
        }
    }
